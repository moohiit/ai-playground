...
/**
 * Session cache for the user's preferences. Mirrors mobile/lib/prefs.ts.
 *
 * Every tab needs the base currency to format money, and each one used to
 * fetch /prefs on mount. Preferences change only from the Settings tab, which
 * invalidates this cache, so one fetch per session is enough. The in-flight
 * promise is cached too, so tabs mounting together share a single request.
 */
