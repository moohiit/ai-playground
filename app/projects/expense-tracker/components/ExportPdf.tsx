...
function filterLine(f: ReportFilters): string {
  ...
  if (f.scope && f.scope !== "all")
    parts.push(f.scope === "personal" ? "Personal only" : "Group only");
...
export function ExportPdfButton({ summary, groupId, groupName, base, filters }: Props) {
  ...
  async function handleExport() {
    ...
    try {
      ...
      y += 5;
      doc.text(`Filters: ${filterLine(activeFilters)}`, pageWidth / 2, y, {
        align: "center",
      });
      y += 5;
      doc.text(
      ...
      if (summary.byCategory.length > 0) {
        ...
        (doc as any).autoTable({
          startY: y,
          head: [["Category", "Count", "Total ", "My Share ", "% of Total"]],
      ...
      if (summary.byMonth.length > 0) {
        ...
        (doc as any).autoTable({
          startY: y,
          head: [["Month", "Count", "Total ", "My Share "]],
      ...
      // Per-group sections carry balances and settlement plans computed across
      // the whole group — they can't honour a category, search or only-mine
      // filter, so they're left out rather than contradicting the figures above.
      const groupsWouldMislead =
        activeFilters.scope === "personal" ||
      ...
      if (isGroupReport) {
        ...
        if (groupsWouldMislead) {
          y = noteOmittedGroups(doc, y, "Member breakdown");
        } else {
          y = await renderGroupSection(doc, y, groupId!, authFetch);
        }
      } else {
        // ─── FULL REPORT (filtered expenses + all groups) ───
        y = await renderAllExpenses(doc, y, activeFilters, authFetch);
        if (groupsWouldMislead) {
          y = noteOmittedGroups(doc, y, "Group breakdowns");
        } else {
...
/** Explains a section left out because it can't reflect the active filters. */
function noteOmittedGroups(doc: any, y: number, heading: string): number {
  ...
  doc.text(
    "Omitted - balances and settlement plans cover every member and category,",
    14,
    y
  );
  y += 4;
  doc.text("so they would not match the filters applied to this report.", 14, y);
  doc.setTextColor(0);
  return y + 10;
}
...
async function renderGroupSection(
  ...
  try {
    const fetches: Promise<Response>[] = [
      ...
      authFetch(`/api/projects/expense-tracker/expenses?groupId=${groupId}&limit=500&settled=all&direction=expense`),
    ];
    if (!group) {
